...
% if _c=='__table__' :
% _caption = _caption if _caption is not None else _lbl
...
Please Login before viewing this page. <span id="login_required"></span>
</span></div>
...
%_autofit = kwoptions.get('autofit',locals().get('grid_autofit','.9-.5')).replace('-',',').split(',')
...
% end
	{{_sep}}{	'name':'{{!_c}}', 'index':'{{!_c}}', 'label':'{{!_lbl}}', 'width': {{str(_w*50) if _w > 0 else '150'}}
		...
		,'formoptions':{'rowpos':{{_frmpos[0]}}, 'colpos':{{_frmpos[1]}} }
		,'search':true
...
var addOption = editOption ; /*{'modal':true, 'width':600, 'labelswidth':'10%'
			,'afterSubmit': function(response, postdata) { return $.parseJSON(response.responseText);}
			,"beforeShowForm" : showform_colspan
			} ;*/
...
%if 'e' in _able :
			$(this).jqGrid('editGridRow',rowid,editOption);
%end
			}
		,"onSpace":function( rowid ) {
%if 'v' in _able :
		 $(this).viewGridRow (rowid,viewOption);
%end
			}
		} )
%if 's' in _able :
...
function exportCSV () {
	...
	$('#expdlg_msg',dlg).html('Select export scope') ;
	...
	$('#exportPage',dlg).html('this page').attr('href',url+q).button();
	qry['rows'] = '-1'; qry['page'] = '1';
	q = $.param(qry); if (q) q = '?'+q ;
	$('#exportAll',dlg).html('all pages').attr('href',url+q).button();
	dlg.dialog({ 
		'modal':true,
		'title': 'export as CSV',
...
$("#grid-list")
	...
	.navButtonAdd('#grid-pager',{
		'caption':"",'title':"export data as CSV",'buttonicon':"ui-icon-disk", 
...
%	qelem += '<input id="qsearch" title="quick search" placeholder="quick search" style="height:1em;" onkeydown="quickSearch(arguments[0]||event);"/>\\n'
%	qelem += '</td>\\n' + sepelem
%else :
%	qelem = '<td></td>\\n'
%end if
